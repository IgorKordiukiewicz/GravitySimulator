	...
	, presetManager(presetManager)
{
	...
	const auto path{ std::filesystem::current_path() / "Screenshots" };
	if (std::filesystem::is_directory(path)) {
		int biggestScreenshotId{ 0 };
		for (const auto& it : std::filesystem::directory_iterator(path)) {
			if (it.path().extension() == ".png") {
				const std::string fileName{ it.path().stem().string() };
...
void Editor::grabBody()
{
	...
	for (CelestialBody& celestialBody : universe.getCelestialBodies()) {
		...
		if (ImGui::CollapsingHeader("Presets")) {
			...
			if (ImGui::Button("Load from preset")) {
				if (selectedPresetName.has_value()) {
					const Preset preset{ presetManager.getPresets().find(selectedPresetName.value())->second };
...
void Editor::updateCelestialBodiesProperties()
{
	...
	for (CelestialBody& celestialBody : universe.getCelestialBodies()) {
		...
		if (ImGui::TreeNode("Properties")) {
			...
			// Update shape color
			const Color color{ celestialBody.getColor() };
